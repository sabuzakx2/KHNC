...
MOBILE_ROOT=/usr/share/khnc/mobile
...
[ -r "$SOURCE_DIR/mobile.html" ] || fail "mobile.html not found in $SOURCE_DIR"
[ -r "$SOURCE_DIR/openwrt/khnc.init" ] || fail "openwrt/khnc.init not found"

mkdir -p "$APP_ROOT" "$CGI_ROOT" "$MOBILE_ROOT" /etc/khnc /etc/khnc/backup-logs /usr/libexec
...
cp "$SOURCE_DIR/version.json" "$MOBILE_ROOT/version.json"
# The mobile server has a separate document root but uses exactly the same
# router APIs as the desktop service, keeping device data and policies shared.
# Copying them avoids uHTTPd builds that refuse CGI paths through symlinks.
if [ -L "$MOBILE_ROOT/cgi-bin" ]; then
  rm -f "$MOBILE_ROOT/cgi-bin"
fi
mkdir -p "$MOBILE_ROOT/cgi-bin"
...
if ! wget -qO- "http://127.0.0.1:$MOBILE_PORT/version.json" >/dev/null 2>&1; then
  "$INIT_SCRIPT" stop || true
  fail "mobile service did not answer on port $MOBILE_PORT; check logread"
fi
...
printf 'Mobile: http://%s:%s\n' "${LAN_IP:-router-ip}" "$MOBILE_PORT"
